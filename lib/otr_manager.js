...
const debug = require('debug')('wire:bot-api:otr_manager');
  ...
  encrypt(userId, clientId, preKey, data) {
    ...
    return new Promise((resolve) => {
      ...
      this.box.session_load(id)
        .then((session) => {
          if (session === null) {
            debug('couldn\'t find session');
            let s;
            this.box.session_from_prekey(id, preKey)
              .then((newSession) => {
                debug(`got session from prekey ${newSession}`);
                s = newSession;
          ...
          if (session === null) {
            ...
            return this.box.session_from_message(id, util.toArrayBuffer(data))
              .then(([newSession, plain]) => {
                plainData = plain;
                debug(`got session ${newSession}`);
                return this.box.session_save(newSession);
              })
              .then(() => {
                debug(`plain ${plainData}`);
