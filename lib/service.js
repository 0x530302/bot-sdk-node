...
const debug = require('debug')('wire:bot-api:service');
...
class Service {
  constructor(options, cb) {
    ...
    https.createServer({ key: fs.readFileSync(options.key),
      ...
      debug(`req: ${req.method} url: ${req.url}`);
  ...
  handleRequest(req, data, res) {
    debug(`Handle request for url ${req.url}`);
    if (req.url === '/bots') {
  ...
  createBot(req, data, res) {
    if (!this.checkAuthHeader(req)) {
      debug(`Invalid auth header, expected ${this.auth}`);
      Service.sendResponse(res, 403);
    } else if (!Service.validateCreateBotData(data)) {
      debug(`Invalid data for creating bot sent. Data: ${JSON.stringify(data)}`);
      Service.sendResponse(res, 404);
    } else {
      debug('Creating new bot...');
  ...
  handleMessages(req, data, res, botID) {
    if (!this.checkAuthHeader(req)) {
      ...
      debug('Invalid data for message handling sent.' +
  ...
  handleMessagesImpl(req, data, res, botID) {
    debug(`Message request for bot ${botID}, data: ${JSON.stringify(data)}`);
  ...
  conversationMessageAdd(req, data, res, botID) {
    ...
    if (!Service.validateConversationMessageAdd(data.data)) {
      debug('Invalid data for conversation message add sent.' +
      `Bot: ${botID}, data: ${JSON.stringify(data.data)}`);
      Service.sendResponse(res, 404);
    } else {
      Service.sendResponse(res, 200);

      debug(`New message for bot ${botID}, data: ${JSON.stringify(data)}`);
      ...
      this.bots[botID].otr.decrypt(data.from, data.data.sender, msg)
        .then((plainText) => {
          ...
          debug(`got plain text ${JSON.stringify(pb)}`);

          this.sendConfirmation(botID, pb.message_id, (confirmationStatus) => {
            debug(`message confirmation sent with status ${confirmationStatus}`);
  ...
  conversationMemberJoin(req, data, res, botID) {
    ...
    if (!Service.validateConversationMemberJoinLeave(data)) {
      debug('Invalid data for conversation member join sent.' +
        `Bot: ${botID}, data: ${JSON.stringify(data)}`);
      Service.sendResponse(res, 404);
    } else {
      debug(`Conversation member join for bot ${botID}, data: ${JSON.stringify(data)}`);
      const newMembers = data.user_ids;
      debug(`New members [${newMembers}] joined conversation` +
      ...
      if (selfAdded) {
        ...
        if (!{}.hasOwnProperty.call(this.bots, 'devices')) {
          debug('will try to retrieve devices for me...');
          this.getDevices(botID, (devices, status) => {
            debug(`getDevices response: ${JSON.stringify(devices)}`);
  ...
  conversationMemberLeave(req, data, res, botID) {
    if (!Service.validateConversationMemberJoinLeave(data)) {
      debug('Invalid data for conversation member leave sent.' +
        `Bot: ${botID}, data: ${JSON.stringify(data)}`);
      Service.sendResponse(res, 404);
    } else {
      debug(`Conversation member leave for bot ${botID}, data: ${JSON.stringify(data)}`);
      const membersToRemove = data.user_ids;
      if (membersToRemove.indexOf(botID) !== -1) { // bot among the user_ids, delete any saved data
        delete this.bots[botID];
      } else {
        debug(`Members [${membersToRemove}] left conversation` +
          `${this.bots[botID].data.conversation.id}`);
  ...
  conversationRename(req, data, res, botID) {
    if (!Service.checkPropertyAndType(data, 'name', 'string')) {
      debug('Invalid data for conversation rename leave sent.' +
        `Bot: ${botID}, data: ${JSON.stringify(data)}`);
      Service.sendResponse(res, 404);
    } else {
      debug(`Conversation rename for bot ${botID}, data: ${JSON.stringify(data)}`);
              ...
              if (pStatus === 200) {
                ...
                Promise.all(p)
                  .then((cyp) => {
                    ...
                    debug(`to send ${JSON.stringify(msg)}`);
  ...
  getAsset(botID, assetID, assetToken, decryptKey, sha256, cb) {
    this.bots[botID].httpsClient.getAsset(assetID, assetToken, (data, status) => {
      ...
      if (status >= 200 && status < 300) {
        ...
        debug('hash match', hb.length, shaBuffer.length, hb.compare(shaBuffer));
        cb(final, status);
      } else {
        cb(null, status);
      }
  ...
  loadState(botID) {
    return new Promise((resolve) => {
      ...
      try {
        ...
        debug(`read from file ${JSON.stringify(data)}`);
        const httpsClient = new HttpsClient(data.token);
        this.bots[botID] = {
          data,
